...
export default function DiagnosisPage() {
  ...
  const handleFileChange = (e) => {
    ...
    reader.onloadend = () => {
      const fullResult = reader.result; // e.g. "data:image/jpeg;base64,/9j/4AAQ..."
      const base64Only = fullResult.split(",")[1]; // strip the "data:image/...;base64," prefix
      setImageBase64(base64Only);
      setImagePreview(fullResult); // keep full string for <img> preview
    };
  ...
  const handleDiagnose = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/diagnosis`, {
        method: "POST",
  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-10">
      ...
      <p className="text-muted mb-8">
        Upload a photo of a sick plant. Our AI will identify the issue and suggest treatment.
      </p>
      ...
      {result && (
        <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-6">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-xl font-bold text-gray-900">{result.diseaseName}</h2>
              {result.scientificName && (
                <p className="italic text-sm text-muted">{result.scientificName}</p>
              )}
            </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className={`rounded-xl p-3 text-center ${severityColor[result.severity] || "bg-gray-50"}`}>
              ...
              <div className="space-y-2">
                {result.possibleCauses.map((c, i) => (
                  <div key={i} className="flex justify-between items-start bg-gray-50 rounded-lg p-3">
                    <div>
                      <p className="font-medium text-sm text-gray-900">{c.cause}</p>
          ...
          {result.suggestedTreatments?.length > 0 && (
            <div>
              <h3 className="font-semibold text-gray-900 mb-2">Suggested Treatments</h3>
              <div className="space-y-2">
                {result.suggestedTreatments.map((t, i) => (
                  <div key={i} className="bg-gray-50 rounded-lg p-3">
                    <p className="font-medium text-sm text-gray-900">
                      {t.name} <span className="text-xs text-muted font-normal">· {t.type}</span>
                    </p>
                    <p className="text-xs text-gray-700 mt-1">{t.instructions}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {result.otherPossibilities?.length > 0 && (
            <div>
              <h3 className="font-semibold text-gray-900 mb-2">Other Possibilities</h3>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-900">
            ⚠️ This AI diagnosis is for educational purposes. For valuable or commercially grown plants, consult a certified plant pathologist.
          </div>
        </div>
      )}
    </main>
