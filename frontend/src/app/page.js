...
<section className="max-w-5xl mx-auto px-6 py-20">
  <div className="grid grid-cols-3 gap-6">
    <div className="bg-primary rotate-2 hover:rotate-0 transition-transform duration-300 rounded-3xl p-6 text-center shadow-lg border-4 border-white">
      <p className="text-4xl font-bold text-white" style={{ fontFamily: "var(--font-display)" }}>35</p>
      <p className="text-xs text-white/70 font-semibold mt-1">Curated plants</p>
    </div>
    <div className="bg-gold -rotate-2 hover:rotate-0 transition-transform duration-300 rounded-3xl p-6 text-center shadow-lg border-4 border-white">
      <p className="text-4xl font-bold text-white" style={{ fontFamily: "var(--font-display)" }}>3</p>
      <p className="text-xs text-white/70 font-semibold mt-1">AI-powered features</p>
    </div>
    <div className="bg-sage rotate-1 hover:rotate-0 transition-transform duration-300 rounded-3xl p-6 text-center shadow-lg border-4 border-white">
      <p className="text-4xl font-bold text-primary" style={{ fontFamily: "var(--font-display)" }}>100%</p>
      <p className="text-xs text-primary/70 font-semibold mt-1">Grounded in real data</p>
    </div>
  </div>
</section>
...
<section className="max-w-6xl mx-auto px-6 py-24 bg-white">
  <div className="text-center mb-14">
    ...
    <h2 className="text-4xl font-bold text-primary" style={{ fontFamily: "var(--font-display)" }}>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
    {features.map((f, i) => {
      const badgeColors = ["bg-primary", "bg-gold", "bg-sage", "bg-clay", "bg-berry", "bg-primary"];
      ...
      return (
        ...
        >
          <div className={`w-14 h-14 rounded-2xl ${badge} flex items-center justify-center text-2xl mb-5 rotate-6 shadow-md border-2 border-white`}>
            {f.icon}
          </div>
          <h3
  ...
  <div className="grid md:grid-cols-2 gap-12 items-center">
    <div className="bg-white border-2 border-primary/10 rounded-3xl shadow-xl p-5 -rotate-1">
      <div className="flex items-center gap-2 mb-4">
        <div className="w-7 h-7 bg-primary rounded-full flex items-center justify-center text-white text-xs">🤖</div>
        <p className="text-sm font-bold text-primary">Nature AI</p>
        <span className="ml-auto text-[10px] text-green-600 font-semibold">● Online</span>
      </div>
      ...
      <div className="bg-cream text-sm text-gray-700 rounded-2xl rounded-tl-sm px-4 py-3 max-w-[92%] leading-relaxed">
        Yes — Tulsi (तुलसी) does well indoors with full sun exposure, moderate
        watering, and loamy soil. It's rated "Easy" difficulty with a fast growth rate.
        <div className="flex gap-1.5 mt-2">
          <span className="text-xs bg-gold/20 text-gold font-semibold px-2 py-0.5 rounded-full">Holy Basil</span>
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <p className="text-gray-600 mb-6 leading-relaxed">
        Every answer is retrieved from our own curated plant database first,
        then used to ground the AI's response — not pulled from general
        internet knowledge.
      </p>
      <div className="space-y-4 mb-8">
        {[
          { icon: "📚", title: "Cited Knowledge Base", desc: "Every response links back to real plants in our database." },
          { icon: "🎯", title: "Semantic Retrieval", desc: "Finds relevant plants by meaning, not just keyword matching." },
          { icon: "🌐", title: "Culturally Aware", desc: "Includes Hindi names and India-relevant plants throughout." },
        ].map((item) => (
      ...
      <Link href="/assistant" className="inline-block bg-primary text-white rounded-full px-6 py-3 font-bold hover:bg-primary-light transition-colors">
        Chat with AI →
      </Link>
    </div>
  </div>
  ...
  <div className="max-w-6xl mx-auto px-6">
    <div className="grid md:grid-cols-2 gap-12 items-center">
      <div>
        ...
        <h2 className="text-3xl font-bold text-primary mb-4" style={{ fontFamily: "var(--font-display)" }}>
          Find your perfect <span className="text-sage">healing plants</span>
        </h2>
        <p className="text-gray-600 mb-6 leading-relaxed">
          Answer a few quick questions about your climate, space, and goals —
          get ranked matches with the real reasoning behind each one.
        </p>
        <div className="grid grid-cols-2 gap-3 mb-8">
          {[
            ...
            <div key={item.label} className="bg-cream rounded-xl p-3 flex items-center gap-2 border border-primary/10">
              <span>{item.icon}</span>
              <span className="text-xs font-semibold text-primary">{item.label}</span>
            </div>
          ))}
        </div>
        <Link href="/recommendations" className="inline-block bg-gold text-white rounded-full px-6 py-3 font-bold hover:opacity-90 transition-opacity">
      ...
      <div className="space-y-3">
        {[
          { name: "Holy Basil", hindi: "तुलसी", tags: "Immunity · Stress · Respiratory", score: 96 },
          { name: "Peppermint", hindi: "पिपरमिंट", tags: "Digestion · Headache", score: 88 },
          { name: "Aloe Vera", hindi: "घृतकुमारी", tags: "Skin Care · Digestion", score: 82 },
        ].map((p, i) => (
          <div
            key={p.name}
          ...
          >
            <div>
              <p className="font-bold text-primary text-sm">{p.name} · {p.hindi}</p>
              <p className="text-xs text-gray-500">{p.tags}</p>
            </div>
            <span className="text-lg font-bold text-gold">{p.score}%</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="grid md:grid-cols-2 gap-10 items-center relative z-10">
      <div>
        ...
        <p className="text-white/70 mb-6 leading-relaxed">
          Upload any plant photo — our AI identifies the species, cross-checks
          our database, and gives you Hindi names and care details instantly.
        </p>
        <Link href="/plant-id" className="inline-block bg-white text-primary rounded-full px-6 py-3 font-bold hover:scale-105 transition-transform duration-200">
          Try Plant ID →
        </Link>
      </div>
      <div className="bg-white/10 backdrop-blur border-2 border-dashed border-white/30 rounded-2xl p-8 text-center rotate-1">
        <p className="text-white/60 text-sm mb-3">📷 Drop a photo or browse</p>
        <div className="bg-white/90 rounded-xl p-3 text-left mt-4">
          <p className="text-primary font-bold text-sm">Peacock Plant</p>
          <p className="text-gray-500 text-xs italic">Goeppertia makoyana · 88% match</p>
        </div>
      </div>
    </div>
    <p
...
<section className="relative bg-primary overflow-hidden">
  <div className="max-w-3xl mx-auto px-6 py-24 text-center relative z-10">
    ...
    >
      ...
      <span className="text-sage">Nature & Nurture puts it in one place</span> —
      grounded in real data, verified with AI, built for people who actually
      want to grow this stuff at home.
    </p>
  </div>
...
<section className="max-w-4xl mx-auto px-6 py-20 text-center">
  <div className="bg-gold rounded-[2.5rem] p-14 -rotate-1 shadow-2xl border-4 border-white">
    ...
    <p className="text-white/80 mb-8">Free to get started. No credit card required.</p>
    <Link
      href="/login"
